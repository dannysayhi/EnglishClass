...
using PCSC;
using SmartCardSystem.Database;
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                // Connect to the card
                    ...
                    string studentID = AuthUUID("20180902", uid);
                    ...
                    if (studentID != "") {
                        ...
                        for (int i = 0; i < pbRecvBuffer.Length; i++)
                            Console.Write("{0:X2} ", pbRecvBuffer[i]);
                        Console.WriteLine();
                        string _get_count = getCount("20180902", studentID);
                            //Console.Write(getCount("20180902", "1"));
                        if (_get_count != "")
                         {
                                studRCagain("20180902", studentID, _get_count);
                                Console.Write(getCount("20180902", studentID).ToString());

                         }
                        else
                        {
                                studRCstart("20180902", studentID, "0", "A");
