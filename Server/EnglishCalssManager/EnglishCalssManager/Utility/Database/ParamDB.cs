...
using System;
using _4RobotSystem.DataBase;

namespace EnglishClassManager.DB
{
    public class ParamDB : ParameterBase
    {
        ...
        public ParamDB(string xmlFullPath)
            : base(xmlFullPath)
        {
        ...
        #region - Open Form -
        frmParamDB _frmParam;
        public void ShowDialog(ParamDB paramDB)
        {
            if ((_frmParam == null) || (_frmParam.IsDisposed))
            {
                _frmParam = new frmParamDB(paramDB);
                _frmParam.Show();
            }
            else
            {
